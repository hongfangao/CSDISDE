...
class CSDI_base(nn.Module):
    def __init__(self, target_dim, config, device):
        ...
        self.sde = VPSDE(
            beta_min=config_diff["beta_start"],
            beta_max=config_diff["beta_end"],
            N=self.num_steps,
            scheduler=config_diff["schedule"]
        )
        
        # # Keep discrete versions for backward compatibility
    ...
    def calc_loss_valid(
        ...
        loss_sum = 0
        for t in range(1, self.num_steps+1):  # calculate loss for all t
            loss = self.calc_loss(
    ...
    def calc_loss(
        ...
        if is_train != 1:  # for validation
            t = torch.ones(B).to(self.device) * (set_t / self.num_steps)
        else:
            t = eps + (1-eps) * torch.rand(B).to(self.device)  # continuous time in [0, 1]
        
        # Continuous-time noise addition using SDE
        mean, std = self.sde.marginal_prob(observed_data, t)
        noise = torch.randn_like(observed_data)
        # Reshape std to match observed_data dimensions (B,K,L)
        std_reshape = std.view(-1, 1, 1)  # (B,1,1)
        noisy_data = mean + std_reshape * noise

        total_input = self.set_input_to_diffmodel(noisy_data, observed_data, cond_mask)

        # Scale time for model input
        scaled_t = t * (self.num_steps - 1)
    ...
    def set_input_to_diffmodel(self, noisy_data, observed_data, cond_mask):
        ...
        if self.is_unconditional == True:
            ...
            # Use reshape with inferred size to maintain element count
            cond_obs = (cond_mask * observed_data).reshape(B, 1, K, L)
            noisy_target = ((1 - cond_mask) * noisy_data).reshape(B, 1, K, L)
    ...
    #         timesteps = torch.linspace(1, 1e-5, self.num_steps + 1).to(self.device)
    ...
    #             dt = (t_prev - t_cur).view(-1, 1, 1)

    #             # Predictor step (Euler-Maruyama)
    #             drift, diffusion = rsde.sde(current_sample, t_cur)
    #             noise = torch.randn_like(current_sample)
    #             diffusion = diffusion.reshape(B, 1, 1).expand_as(current_sample)
    #             step_size = torch.sqrt(dt).expand_as(current_sample)
    #             predictor = current_sample + drift * dt + diffusion * step_size * noise

    #             # Corrector step (Langevin dynamics)
    #             score = self.diffmodel(
    #                 self.set_input_to_diffmodel(predictor, observed_data, cond_mask),
    #                 side_info,
    #                 t_cur
    #             )
    #             noise = torch.randn_like(predictor)
    #             current_sample = predictor + 0.5 * diffusion**2 * score * dt + diffusion * step_size * noise
    ...
    def impute(self, observed_data, cond_mask, side_info, n_samples):
        ...
        for i in range(n_samples):
            ...
            for t in range(1, len(timesteps)):
                t_cur = torch.full((B,), timesteps[t].item(), device=self.device)
                t_prev = torch.full((B,), timesteps[t - 1].item(), device=self.device)
                dt = (t_prev[0] - t_cur[0])  # scalar

                # ----- Corrector step: Langevin MCMC at time t -----
                score = self.diffmodel(
                    self.set_input_to_diffmodel(current_sample, observed_data, cond_mask),
                    side_info,
                    t_cur
                )
                noise = torch.randn_like(current_sample)
                ...
                drift, diffusion = rsde.sde(current_sample, t_cur)
                noise = torch.randn_like(current_sample)
                diffusion = diffusion.view(B, 1, 1)
                current_sample = current_sample - drift * dt + diffusion * torch.sqrt(dt) * noise
